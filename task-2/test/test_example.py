...
class TestExample(DbTest):
    ...
    def test_count_the_number_of_subordinates(self, conn):
        ...
        sql = """
        SELECT COUNT(s.*) AS subordinates_count, o.id AS id FROM organizations AS o
        LEFT JOIN enterprise_sales_enterprise_customers AS s 
        ON o.id = s.sales_organization_id
        GROUP BY o.id
        ORDER BY o.id ASC;
        """
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
    ...
    def test_segments_using_geojson_boundary(self, conn):
        ...
        SELECT id
        FROM   japan_segments
        WHERE  japan_segments.bounds && ST_MakeEnvelope (
